...
import * as XLSX from 'xlsx'; // Asegúrate de importar XLSX si lo usas en alguna parte del código
...
const Dashboard = () => {
  ...
  const fetchPrices = async () => {
    try {
      const response = await fetch('https://api.coingecko.com/api/v3/simple/price?ids=bitcoin,ethereum&vs_currencies=usd,eur&include_24h_change=true', {
        mode: 'cors', // Asegúrate de manejar CORS correctamente
      });
      if (!response.ok) {
  ...
  useEffect(() => {
    fetchPrices();
    // Cambia el intervalo a un tiempo mayor para evitar "Too Many Requests"
    const interval = setInterval(fetchPrices, 300000); // 300000ms = 5 minutos
