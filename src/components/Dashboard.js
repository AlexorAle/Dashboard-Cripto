...
const Dashboard = () => {
  ...
  const fetchPrices = async () => {
    try {
      ...
      setPrices({
        ...
        'ETH_EUR': { price: data.ethereum.eur, change: data.ethereum.eur_24h_change }, // Added ETH_EUR
      });
    } catch (error) {
  ...
  }, []); // Removed prices dependency to avoid unnecessary re-renders
    ...
    <div className="dashboard-container">
      <div className={`sidebar ${sidebarCollapsed ? 'collapsed' : ''}`}>
        ...
        <ul className="sidebar-menu">
          <li onClick={() => setCurrentSection('dashboard')} className={currentSection === 'dashboard' ? 'active' : ''}>
            <BarChart2 size={24} />
            {!sidebarCollapsed && <span>Cryptocurrency Dashboard</span>}
          </li>
          <li onClick={() => setCurrentSection('investments')} className={currentSection === 'investments' ? 'active' : ''}>
            <Briefcase size={24} />
