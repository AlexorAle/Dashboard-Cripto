...
const DashboardView = ({ prices, renderOperationsTable, formatNumber, formatPercentage, readExcel }) => {
  ...
  return (
    <div className="dashboard bg-gray-900 text-white min-h-screen">
      ...
      <section className="prices-section p-4">
        <h2 className="text-2xl mb-4">Real-Time Quotes</h2>
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          {Object.entries(prices).map(([key, { price, change }]) => (
            <div key={key} className="crypto-box p-4 bg-gray-800 rounded-lg shadow-lg">
